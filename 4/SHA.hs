...
data SHA1State = SHA1S !Word32 !Word32 !Word32 !Word32 !Word32 deriving (Eq, Show)

initialSHA1State :: SHA1State
initialSHA1State = SHA1S 0x67452301 0xefcdab89 0x98badcfe 0x10325476 0xc3d2e1f0

fromState (SHA1S a b c d e) = [a,b,c,d,e]
...
sha1cc :: SHA1State -> ByteString -> SHA1State
sha1cc state bs_in = fstate
 where
  bs_pad = padSHA1 bs_in
  fstate = runSHA state processSHA1Block bs_pad

sha1cc' :: SHA1State -> ByteString -> SHA1State
sha1cc' state bs_in = fstate
 where
  fstate = runSHA state processSHA1Block bs_in
